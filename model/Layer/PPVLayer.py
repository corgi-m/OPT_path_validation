...
class PPVLayer(ABCLayer, PPVHelper):
    ...
    def PPV_R_sign(self, package, PATH, index):
        ...
        if package.is_mf1_empty():
            L = len(PATH[1:-1]) - index
            if L == 0:
                return True
            P = 1 / L
            if check_probability(P):
                message = strcat(PATH[0], package.get_TTL(), PATH[index - 1], package.get_flowid())
                Mi = self.MAC(self.Ki[package.get_flowid()][PATH[-1]], message)
                message = strcat(package.get_mvf(), PATH[index], Mi, PATH[index + 1])
                mvf1 = self.MAC(self.Ki[package.get_flowid()][PATH[-1]], message)
                package.set_add1(PATH[index])
                package.set_mf1(Mi)
                package.set_mvf(mvf1)
        else:
            if package.is_mf2_empty():
                message = strcat(PATH[0], package.get_TTL(), PATH[index - 1], package.get_flowid())
                Mi = self.MAC(self.Ki[package.get_flowid()][PATH[-1]], message)
                message = strcat(package.get_mvf(), Mi)
                mvf2 = self.MAC(self.Ki[package.get_flowid()][PATH[-1]], message)
                package.set_add2(PATH[index])
                package.set_mf2(Mi)
                package.set_mvf(mvf2)
        return True
